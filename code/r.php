<?php
// =====================
// 1️⃣ دالة جلب جميع المبلغين مع عدد البلاغات
// =====================
function getReportersWithReportCount() {
    ...
    $sql = "
        SELECT 
            ...
            GROUP_CONCAT(DISTINCT rr.report_id) as report_ids
        FROM reporters r
        LEFT JOIN districts d ON r.districts_id = d.id
        LEFT JOIN neighborhoods n ON r.neighborhoods_id = n.id
        LEFT JOIN report_reporter rr ON r.id = rr.reporter_id
        GROUP BY r.id
...
// =====================
// 2️⃣ دالة جبل المبلغ مع البلاغات المفصلة
// =====================
function getReporterWithReports($reporterId) {
    $conn = new mysqli("localhost:3306", "root", "root", "departments_system");
    ...
    $stmt = $conn->prepare("
        SELECT 
            ...
            rm.type_name as main_type,
            rs.sub_type_name as sub_type,
            dep.department_name,
            d.name as report_district,
            n.name as report_neighborhood
        FROM reports rep
        INNER JOIN report_reporter rr ON rep.report_id = rr.report_id
        LEFT JOIN report_main_types rm ON rep.main_id = rm.id
        LEFT JOIN report_sub_types rs ON rep.sub_id = rs.id
        LEFT JOIN report_department rd ON rep.report_id = rd.report_id
        LEFT JOIN department dep ON rd.department_id = dep.id
        LEFT JOIN districts d ON rep.districts_id = d.id
        LEFT JOIN neighborhoods n ON rep.neighborhoods_id = n.id
    ...
    $reporter['total_reports'] = count($reports);
...
// =====================
// 3️⃣ دالة البحث عن المبلغين بالاسم أو الهاتف
// =====================
function searchReporters($searchTerm) {
    $conn = new mysqli("localhost:3306", "root", "root", "departments_system");
    if ($conn->connect_error) return [];
    $conn->set_charset("utf8mb4");
    ...
    $stmt = $conn->prepare("
        ...
        LEFT JOIN neighborhoods n ON r.neighborhoods_id = n.id
        LEFT JOIN report_reporter rr ON r.id = rr.reporter_id
        WHERE r.name_reporter LIKE ? OR r.phone_reporter LIKE ? OR r.id_national_reporter LIKE ?
        GROUP BY r.id
        ORDER BY r.name_reporter
    ");
...
// =====================
// 4️⃣ دالة معالجة العمليات للمبلغين
// =====================
function handleReportersOperation($operation, $data = null) {
    switch ($operation) {
        case "show":
            echo json_encode(getReportersWithReportCount());
            break;
            
        case "get":
            if (isset($_GET['id'])) {
                echo json_encode(getReporterWithReports($_GET['id']));
            } else {
                echo json_encode(["status" => "error", "message" => "لم يتم تحديد ID"]);
            }
            break;
            
        case "search":
            if (isset($_GET['term'])) {
                echo json_encode(searchReporters($_GET['term']));
            } else {
                echo json_encode(["status" => "error", "message" => "لم يتم تحديد مصطلح البحث"]);
            }
            break;
...
function getAllDepartmentReports($id = null, $limit = 5, $status = null) {
    $conn = new mysqli("localhost:3306", "root", "root", "departments_system");
    if ($conn->connect_error) return [];
    $conn->set_charset("utf8mb4");
    ...
    $sql = "
        ...
        LEFT JOIN department d ON rd.department_id = 1
    ";
    ...
    if ($id !== null) {
        $id = (int)$id;
        $whereConditions[] = "r.report_id = $id";
    }
...
/**
 * 🔍 جلب جميع البلاغات التابعة لقسم معيّن
 *
 * @param int|null $departmentId   رقم القسم
 * @param int|null $limit          عدد السجلات المراد جلبها (اختياري)
 * @param string|null $status      حالة البلاغ (اختياري)
 * @return array                   قائمة البلاغات
 */
 /*
...
WHERE d.id = 1  -- شرط لتصفية القسم 1
ORDER BY r.created_at DESC
LIMIT 25
";
...
function operationReports($operation, $data = null) {
    switch ($operation) {
        ...
        if(isset($_GET['data']) && isset($_GET['data']['limit'])) {
          $da = $_GET['data'];
          $limit = $da["limit"];
           echo json_encode(getAllDepartmentReports(null,$limit));
        } else {
        echo json_encode(getAllDepartmentReports());
        }
         break;
        default:
